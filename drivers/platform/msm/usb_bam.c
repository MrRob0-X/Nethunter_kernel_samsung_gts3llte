...
static void usb_bam_finish_suspend(enum usb_ctrl cur_bam)
{
	...
	if (info[cur_bam].pipes_suspended == ctx->pipes_enabled_per_bam &&
		...
		ipa_rm_notify_completion(
			...
			(enum ipa_rm_resource_name)ipa_rm_resource_cons[cur_bam]);
	}
...
static void usb_bam_ipa_create_resources(enum usb_ctrl cur_bam)
{
	...
	usb_prod_create_params.name = (enum ipa_rm_resource_name)ipa_rm_resource_prod[cur_bam];
	...
	usb_cons_create_params.name = (enum ipa_rm_resource_name)ipa_rm_resource_cons[cur_bam];
...
static void wait_for_prod_granted(enum usb_ctrl cur_bam)
{
	...
	ret = ipa_rm_request_resource((enum ipa_rm_resource_name)ipa_rm_resource_prod[cur_bam]);
	if (!ret) {
		info[cur_bam].cur_prod_state = IPA_RM_RESOURCE_GRANTED;
		complete_all(&info[cur_bam].prod_avail);
			      ...
			      enum usb_device_speed usb_connection_speed)
{
	...
	if (dir == USB_TO_PEER_PERIPHERAL) {
		log_event_dbg("%s: vote ipa_perf resource=%d perf=%d mbps\n",
			...
			ipa_rm_perf_prof.max_supported_bandwidth_mbps);
		ret = ipa_rm_set_perf_profile((enum ipa_rm_resource_name)ipa_rm_resource_prod[cur_bam],
					&ipa_rm_perf_prof);
	} else {
		log_event_dbg("%s: vote ipa_perf resource=%d perf=%d mbps\n",
			__func__, ipa_rm_resource_cons[cur_bam],
			ipa_rm_perf_prof.max_supported_bandwidth_mbps);
		ret = ipa_rm_set_perf_profile((enum ipa_rm_resource_name)ipa_rm_resource_cons[cur_bam],
					&ipa_rm_perf_prof);
	}
